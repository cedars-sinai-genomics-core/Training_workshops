...
Luckily the data for the differential expression testing is in a much more plotting friendly format to begin with, so we won't need to do nearly as much reshaping.
...
Since we plan to merge to two data sets, we need to have a column with common values between them. In this case we can use the gene symbol as the common column, but we'll need to split the ENSEMBL IDs and symbols apart for this dataset too. Here we'll use a new argument `.before=1` to place the new column at the left of the data frame.
